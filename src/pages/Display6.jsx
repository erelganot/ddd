...
export default function Display6() {
  ...
  const loadGenerations = async () => {
    ...
    try {
      ...
      const display6BaseImages = baseImages.filter(img => img.display_page === 'display6');
      const display6BaseImageIds = display6BaseImages.map(img => img.id);

      const display6Generations = allGeneratedImages.filter(gen => 
        display6BaseImageIds.includes(gen.base_image_id) && 
        gen.status === 'completed' && 
        gen.generated_image_url
      );

      const enrichedGenerations = display6Generations.map(gen => ({
        ...gen,
      ...
      console.error("[Display6] Error loading generations:", error);
    }
  };
  ...
  const pollPendingImages = async () => {
    ...
    try {
      ...
      const imagesToPoll = allGenerations.filter(gen => {
        const isDisplay6Image = display6BaseImageIds.includes(gen.base_image_id);
        ...
        return isDisplay6Image && needsPolling && hasPollUrl && belowMaxAttempts;
      });
      ...
      for (const image of imagesToPoll) {
        ...
        try {          
          ...
          console.error(`[Display6] Error polling image ${image.id}:`, pollError);
        }
      }

    } catch (error) {
      console.error("[Display6] Error during polling check:", error);
    }
  };
        ...
        {currentGeneration ? (
          ...
          >
            ...
            <h1 className="text-4xl font-bold text-white mb-4">
              Display 6 - Waiting for Magic
            </h1>
            <p className="text-xl text-white/60 max-w-md mx-auto">
              Generated artworks assigned to Display 6 will appear here
            </p>
          </motion.div>
        )}
      </AnimatePresence>
