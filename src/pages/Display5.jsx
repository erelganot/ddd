...
export default function Display5() {
  ...
  const loadGenerations = async () => {
    ...
    try {
      ...
      const display5BaseImages = baseImages.filter(img => img.display_page === 'display5');
      const display5BaseImageIds = display5BaseImages.map(img => img.id);

      const display5Generations = allGeneratedImages.filter(gen => 
        display5BaseImageIds.includes(gen.base_image_id) && 
        gen.status === 'completed' && 
        gen.generated_image_url
      );

      const enrichedGenerations = display5Generations.map(gen => ({
        ...gen,
      ...
      console.error("[Display5] Error loading generations:", error);
    }
  };
  ...
  const pollPendingImages = async () => {
    ...
    try {
      ...
      const imagesToPoll = allGenerations.filter(gen => {
        const isDisplay5Image = display5BaseImageIds.includes(gen.base_image_id);
        ...
        return isDisplay5Image && needsPolling && hasPollUrl && belowMaxAttempts;
      });
      ...
      for (const image of imagesToPoll) {
        ...
        try {          
          ...
          console.error(`[Display5] Error polling image ${image.id}:`, pollError);
        }
      }

    } catch (error) {
      console.error("[Display5] Error during polling check:", error);
    }
  };
        ...
        {currentGeneration ? (
          ...
          >
            ...
            <h1 className="text-4xl font-bold text-white mb-4">
              Display 5 - Waiting for Magic
            </h1>
            <p className="text-xl text-white/60 max-w-md mx-auto">
              Generated artworks assigned to Display 5 will appear here
            </p>
          </motion.div>
        )}
      </AnimatePresence>
