...
export default function Display4() {
  ...
  const loadGenerations = async () => {
    ...
    try {
      ...
      const display4BaseImages = baseImages.filter(img => img.display_page === 'display4');
      const display4BaseImageIds = display4BaseImages.map(img => img.id);

      const display4Generations = allGeneratedImages.filter(gen => 
        display4BaseImageIds.includes(gen.base_image_id) && 
        gen.status === 'completed' && 
        gen.generated_image_url
      );

      const enrichedGenerations = display4Generations.map(gen => ({
        ...gen,
      ...
      console.error("[Display4] Error loading generations:", error);
    }
  };
  ...
  const pollPendingImages = async () => {
    ...
    try {
      ...
      const imagesToPoll = allGenerations.filter(gen => {
        const isDisplay4Image = display4BaseImageIds.includes(gen.base_image_id);
        ...
        return isDisplay4Image && needsPolling && hasPollUrl && belowMaxAttempts;
      });
      ...
      for (const image of imagesToPoll) {
        ...
        try {          
          ...
          console.error(`[Display4] Error polling image ${image.id}:`, pollError);
        }
      }

    } catch (error) {
      console.error("[Display4] Error during polling check:", error);
    }
  };
        ...
        {currentGeneration ? (
          ...
          >
            ...
            <h1 className="text-4xl font-bold text-white mb-4">
              Display 4 - Waiting for Magic
            </h1>
            <p className="text-xl text-white/60 max-w-md mx-auto">
              Generated artworks assigned to Display 4 will appear here
            </p>
          </motion.div>
        )}
      </AnimatePresence>
