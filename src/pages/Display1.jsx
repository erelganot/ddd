...
export default function Display1() {
  ...
  const loadGenerations = async () => {
    ...
    console.log("[Display1] Loading generations...");
    try {
      ...
      console.log("[Display1] All base images:", baseImages);
      console.log("[Display1] All generated images:", allGeneratedImages.length);

      // Find base images assigned to display1
      const display1BaseImages = baseImages.filter(img => img.display_page === 'display1');
      console.log("[Display1] Base images for display1:", display1BaseImages);
      
      const display1BaseImageIds = display1BaseImages.map(img => img.id);
      console.log("[Display1] Base image IDs for display1:", display1BaseImageIds);

      // Filter generated images for display1 base images
      const display1Generations = allGeneratedImages.filter(gen => 
        display1BaseImageIds.includes(gen.base_image_id) && 
        gen.status === 'completed' && 
        gen.generated_image_url
      );
      
      console.log("[Display1] Filtered generations for display1:", display1Generations.length);

      const enrichedGenerations = display1Generations.map(gen => ({
        ...gen,
      ...
      setDebugInfo({
        ...
        display1BaseImages: display1BaseImages.length,
        totalGeneratedImages: allGeneratedImages.length,
        display1GeneratedImages: display1Generations.length,
        display1BaseImageNames: display1BaseImages.map(img => img.name)
      });
      
      if(isMounted.current) setGenerations(enrichedGenerations);
      
    } catch (error) {
      console.error("[Display1] Error loading generations:", error);
    }
  };
  ...
  const pollPendingImages = async () => {
    ...
    try {
      ...
      const imagesToPoll = allGenerations.filter(gen => {
        ...
        const isDisplay1 = display1BaseImageIds.includes(gen.base_image_id);
        const hasPollUrl = !!gen.poll_url;
        const attempts = gen.poll_attempts || 0;
        const belowMaxAttempts = attempts < MAX_POLL_ATTEMPTS;
        
        return needsPolling && isDisplay1 && hasPollUrl && belowMaxAttempts;
      });
      ...
      for (const image of imagesToPoll) {
        ...
        try {          
          ...
          if (responseData && responseData.success && responseData.is_final && isMounted.current) {
            await loadGenerations(); // Reload display list
          }

        } catch (pollError) {
          console.error(`[Display1] Error polling image ${image.id}:`, pollError);
        }
      }

    } catch (error) {
      console.error("[Display1] Error during polling check:", error);
    }
  };
  ...
  return (
    <div className="h-screen bg-black flex items-center justify-center">
      ...
      <div className="absolute top-4 left-4 text-white text-xs bg-black/70 p-2 rounded z-50">
        <div>Display1 Debug:</div>
        <div>Base Images for display1: {debugInfo.display1BaseImages || 0}</div>
        <div>Generated Images: {debugInfo.display1GeneratedImages || 0}</div>
        <div>Base Image Names: {debugInfo.display1BaseImageNames?.join(', ') || 'None'}</div>
        <div>Current Index: {currentIndex}</div>
        <div>Total Generations: {generations.length}</div>
      </div>
        ...
        {currentGeneration ? (
          ...
          >
            ...
            <h1 className="text-4xl font-bold text-white mb-4">
              Display 1 - Waiting for Images
            </h1>
            <p className="text-xl text-white/60 max-w-md mx-auto">
              Images assigned to Display 1 will appear here
            </p>
            <div className="mt-4 text-sm text-white/40">
              Base images for Display 1: {debugInfo.display1BaseImageNames?.join(', ') || 'None assigned'}
            </div>
          </motion.div>
        )}
      </AnimatePresence>
