...
export default function Display2() {
  ...
  const loadGenerations = async () => {
    ...
    try {
      ...
      const display2BaseImages = baseImages.filter(img => img.display_page === 'display2');
      const display2BaseImageIds = display2BaseImages.map(img => img.id);

      const display2Generations = allGeneratedImages.filter(gen => 
        display2BaseImageIds.includes(gen.base_image_id) && 
        gen.status === 'completed' && 
        gen.generated_image_url
      );

      const enrichedGenerations = display2Generations.map(gen => ({
        ...gen,
      ...
      console.error("[Display2] Error loading generations:", error);
    }
  };
  ...
  const pollPendingImages = async () => {
    ...
    try {
      ...
      const imagesToPoll = allGenerations.filter(gen => {
        const isDisplay2Image = display2BaseImageIds.includes(gen.base_image_id);
        ...
        return isDisplay2Image && needsPolling && hasPollUrl && belowMaxAttempts;
      });
      ...
      for (const image of imagesToPoll) {
        ...
        try {          
          ...
          console.error(`[Display2] Error polling image ${image.id}:`, pollError);
        }
      }

    } catch (error) {
      console.error("[Display2] Error during polling check:", error);
    }
  };
        ...
        {currentGeneration ? (
          ...
          >
            ...
            <h1 className="text-4xl font-bold text-white mb-4">
              Display 2 - Waiting for Magic
            </h1>
            <p className="text-xl text-white/60 max-w-md mx-auto">
              Generated artworks assigned to Display 2 will appear here
            </p>
          </motion.div>
        )}
      </AnimatePresence>
